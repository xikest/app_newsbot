...
class SrcRss:
    ...
    async def generator(self) -> Context:
        ...
        for rss in self._rssList:
            try:
                logging.info(f"Start getting the feed from the {rss.name}'s: {datetime.datetime.now()}")
                ...
                logging.info(f"Finished obtaining the feed from the {rss.name}'s : {datetime.datetime.now()}")

            except Exception as e:
                time_sleep = datetime.datetime.now()
                if self.verbose:
                    logging.error(f'Error description -> {e}')
                    logging.error(f"Raised a feed error from the {rss.name}'s @{time_sleep}")
                await asyncio.sleep(60 * 10)  # 다음 반복까지 대기 시간
                time_awake = datetime.datetime.now()
                if self.verbose:
                    logging.error(f"Awakened a feed error from the {rss.name}'s @{time_awake}")
